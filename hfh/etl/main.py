    ...
    try:
        ...
        if imgs:
            popen('ssh -tt -i {vPEM} ubuntu@{vINIT_IP} "sudo tar czf \
                  ...
                  .format(vPEM=PEM, vINIT_IP=INIT_IP, vINIT_LOC=INIT_LOC,
                          vTAR=TAR))
        else:
            popen('ssh -tt -i {vPEM} ubuntu@{vINIT_IP} "sudo tar czf \
                    {vINIT_LOC}/{vTAR} -C {vINIT_LOC}/submissions .'
        ...
        popen('scp -r -i {vPEM} ubuntu@{vINIT_IP}:{vINIT_LOC}/{vTAR}'
              ' {vDEST_LOC}/{vDV}/{vTAR}'
              .format(vPEM=PEM, vINIT_IP=INIT_IP, vINIT_LOC=INIT_LOC, vDV=DV,
                      vTAR=TAR, vDEST_LOC=DEST_LOC))
...
def _break_up_list(k, v, list):
    """if we want to keep repeating vals in
       same table as opposed to rel... currently unused"""
    list_jc = []
...
#         returns structured json content and lists for relational
#         table creation"""
#     # TODO: param to decide if creating relational
#     #       tables or keeping in same table
#     new_jc = OrderedDict()
#     rel = []
#
...
def find_rels(cont):
    """iterate through all entries and identify which
       columns contain a list and are relational entries
...
def _break_up(cont):
    """take in full contents, identify relational entries (ie lists)
        and add them to a seperate list for insertion,
        dicts for grouped questions and individual items
        input: cont (list of dictionaries)
        return: cont (list of dictionaries without rels),
        rel (relatinonal entries)"""
    ...
    for v in cont:
        ...
        for ik, iv in v.iteritems():
            ...
            if ik in cols:
                if not isinstance(iv, list):
                    # if there is only one entry for a relational
                    # item we need to make it a list so it can be traversed
                    iv = [iv]

                for ind in iv:
                    # add in additional data
                    ind['general_info_registration_number']\
                            = v['general_info']['registration_number']
        ...
        # if we have added any non-relational
        # items... should probably always be true
        if add_ent:
