...
def _osm_wn_qry(ids, worn):
    """structure query to send to OSM - generates for
       multiple ID looks up in one call
        input: list of IDs, 'way'/'node'
        return: query to use for OSM call
    """
    return '(' + ''.join(
                    map(str,
                        ['%s(%s);' % (worn, v) for v in ids])) + ')'
...
def _columnify_osm_res(res, worn):
    """turn a dict of osm results into a dict that can be used for db insertion
        ...
        return: dict with values of colname:value
            colname structured with layers of dict
            ie geometry_cooredinates or properties_building

            sample res: {"features": [{"geometry": {"coordinates": [,],
                                      "type": "Point"},
    ...
    for k, v in res.iteritems():
        if not isinstance(v, dict):
            ...
            for sk, sv in v.iteritems():
                # some property keys have colons which won't work
                # as col names, so replace with _
                coldict['%s_%s' % (k, sk.replace(':', '_'))] = sv

    if worn == 'way':
        coldict['geometry_coordinates']\
                = _geomify_way(coldict['geometry_coordinates'])
    elif worn == 'node':
        coldict['geometry_coordinates']\
                = _geomify_node(coldict['geometry_coordinates'])
        pass
...
def _qry_osm(ids, worn):
    """appropriately query OSM. we can only query a
       max of 1023 elements at a time"""
    api = overpass.API()

    SKIP = 1023
    ret = []
    for i in xrange(0, len(ids)-1, 1023):
        ret += api.Get(_osm_wn_qry([str(v) for v in ids[i: i+SKIP]],
                                   worn))['features']
        # sleep so that we don't get multiple requests error
        time.sleep(5)

    # TODO: better way to terminate overpass connections
...
def _get_osm(indict, worn):
    ...
    """return a dictionary of node and way over pass data
       with prefix for a series of wn IDs
    ...
    retdict = {}
    # use the value of indict to match with key of
    # redict to match osm data with proper uuid
    for k, v in indict.iteritems():
        if v in middict:
...
def _get_max_nw(osment):
    """get ids for most recently added nodes or ways
        ...
        output: a dict containing {'way' : id, 'node': id}
        if the given element is in the osm string
    """
    tsformat = '%Y-%m-%dT%H:%M:%SZ'
    curosm = xml.dom.minidom.parseString(
                osment.replace("'", "\"")).documentElement

    attdict = [{'id': e.getAttribute('id'),
                'ts': time.mktime(datetime.datetime.strptime(
                            e.getAttribute('timestamp'),
                            tsformat)
                       .timetuple()), 'type': e.localName}
               for e in curosm.childNodes]
...
def _get_osm_id(indict):
    """extract osm ids from >=1 list of .osm files contents
       - only reads most recent entry
...
def update_all_osm(schema, survey_nm, uuidcol, wcol, ncol):
    # TODO: handle if column doesnt' exist
    # (currently just get None object returned)
    # TODO: set existing values to Null if new one is found
    # so that you don't have leftovers  from previous entry
    ...
    waydict = dict(zip(db.get_column(survey_nm, uuidcol),
                       db.get_column(survey_nm, wcol)))
    nodedict = dict(zip(db.get_column(survey_nm, uuidcol),
                        db.get_column(survey_nm, ncol)))
...
def store_an_osm(cont, survey_nm):
    ...
    multidict = {v['meta_instanceId']: v['local_osm_data']
                 for v in cont if 'local_osm_data' in v}

    waydict, nodedict = _get_osm_id(multidict)

    if waydict:
        logger.info("Pulling way data for %i ways for %s"
                    % (len(waydict.keys()), survey_nm))
        _push_element(_get_osm(waydict, 'way'), survey_nm)

    if nodedict:
        logger.info("Pulling node data for %i nodes for %s"
                    % (len(nodedict.keys()), survey_nm))
        _push_element(_get_osm(nodedict, 'node'), survey_nm)
